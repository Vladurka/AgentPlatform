    ...
    <div style={{ backgroundColor: '#0F0F13', fontFamily: '"Inter", system-ui, sans-serif', fontSynthesis: 'none', minHeight: '100vh', MozOsxFontSmoothing: 'grayscale', overflow: 'clip', position: 'relative', WebkitFontSmoothing: 'antialiased' }}>
      ...
      <nav style={{ alignItems: 'center', backdropFilter: 'blur(20px)', backgroundColor: '#0F0F13B3', borderBottom: '1px solid #FFFFFF0D', display: 'flex', height: '68px', left: 0, paddingInline: '64px', position: 'absolute', right: 0, top: 0, zIndex: 10 }}>
        ...
        <div style={{ marginLeft: 'auto', alignItems: 'center', display: 'flex', gap: '12px' }}>
          <span style={{ color: '#8888AA', fontSize: '14px' }}>Already have an account?</span>
          <Link to="/login" style={{ color: '#A78BFA', fontSize: '14px', fontWeight: 600, textDecoration: 'none' }}>Sign in</Link>
        </div>
      </nav>
      ...
      <div style={{ alignItems: 'center', display: 'flex', justifyContent: 'center', minHeight: '100vh', paddingTop: '68px', position: 'relative', zIndex: 1 }}>
        <div style={{ backgroundColor: '#18181FE6', border: '1px solid #FFFFFF14', borderRadius: '20px', boxShadow: '#00000066 0px 32px 80px, #7C3AED1A 0px 0px 0px 1px', padding: '40px', width: '100%', maxWidth: '400px' }}>
          ...
          <div style={{ alignItems: 'center', display: 'flex', flexDirection: 'column', marginBottom: '32px' }}>
            ...
            <h1 style={{ color: '#E2E2F0', fontSize: '22px', fontWeight: 700, letterSpacing: '-0.4px', margin: 0 }}>Create your account</h1>
            <p style={{ color: '#8888AA', fontSize: '14px', margin: '6px 0 0' }}>Start building AI agents for free</p>
          </div>

          {/* OAuth buttons */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '8px' }}>
            <a href={`${import.meta.env.VITE_API_URL ?? 'http://localhost:5000/api/v1'}/auth/google`}
              style={{ alignItems: 'center', backgroundColor: '#FFFFFF0A', border: '1px solid #FFFFFF14', borderRadius: '10px', color: '#E2E2F0', cursor: 'pointer', display: 'flex', fontSize: '14px', fontWeight: 500, gap: '10px', justifyContent: 'center', padding: '11px', textDecoration: 'none', transition: 'background 0.15s' }}
              onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = '#FFFFFF14')}
            ...
            <a href={`${import.meta.env.VITE_API_URL ?? 'http://localhost:5000/api/v1'}/auth/github`}
              style={{ alignItems: 'center', backgroundColor: '#FFFFFF0A', border: '1px solid #FFFFFF14', borderRadius: '10px', color: '#E2E2F0', cursor: 'pointer', display: 'flex', fontSize: '14px', fontWeight: 500, gap: '10px', justifyContent: 'center', padding: '11px', textDecoration: 'none', transition: 'background 0.15s' }}
              onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = '#FFFFFF14')}
              onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = '#FFFFFF0A')}
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#E2E2F0"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z"/></svg>
              Continue with GitHub
            </a>
          </div>

          <div style={{ alignItems: 'center', display: 'flex', gap: '12px', marginBottom: '8px' }}>
            <div style={{ backgroundColor: '#FFFFFF14', flex: 1, height: '1px' }} />
            <span style={{ color: '#44445A', fontSize: '12px' }}>or</span>
            <div style={{ backgroundColor: '#FFFFFF14', flex: 1, height: '1px' }} />
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div>
              <label style={{ color: '#8888AA', display: 'block', fontSize: '12px', fontWeight: 500, marginBottom: '6px' }}>Confirm password</label>
              <input
                type={showPw ? 'text' : 'password'}
                autoComplete="new-password"
                value={confirm}
